...
async def get_id_user(request: Request, user_id: Annotated[int, Path(ge=1)]):
    for i_user in users:
        if i_user.id == user_id:
            return templates.TemplateResponse("main.html", {"request": request, "user": i_user})
    raise HTTPException(status_code=404, detail="Нету вашего пользователя.")
